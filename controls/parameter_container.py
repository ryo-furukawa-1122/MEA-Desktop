...
class ParameterContainer(ft.Row):
    def __init__(self):
        ...
        self.stim_onset = ft.TextField(label="Stim. onset (ms)", value="50", text_align=ft.TextAlign.LEFT, width=200)
        self.stim_ch = ft.TextField(label="Stim. ch", value="43", text_align=ft.TextAlign.LEFT, width=150)
        self.scale = ft.TextField(label="Scale (\u03bcV)", value="80", text_align=ft.TextAlign.LEFT, width=150)
        super().__init__([self.trials, self.stim_onset, self.stim_ch, self.scale])
